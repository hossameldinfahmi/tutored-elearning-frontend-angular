...
export class QuestionService {
  ...
  getAllQuestionExam(id: number): Observable<any> {
    const token: string = localStorage.getItem("Authorization")!;
    const headers = new HttpHeaders({
      Authorization: token,
    });
  ...
  getoneQestion(id: number): Observable<any> {
    const token: string = localStorage.getItem("Authorization")!;
    const headers = new HttpHeaders({
      Authorization: token,
    });
  ...
  CreateQestion(data: any, id: any): Observable<any> {
    const token: string = localStorage.getItem("Authorization")!;
    ...
    return this.httpClient.post<{
      ...
      status: boolean;
      error: any;
    }>(`${environment.baseUrl}exams/${id}/questions`, data, {
      headers,
    });
  }
